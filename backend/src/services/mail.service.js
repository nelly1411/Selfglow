...
// ─── Transporter (Gmail-Beispiel, .env-Werte) ─────────────────────────────────
// Für andere Anbieter: host/port anpassen
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: process.env.MAIL_USER, // z.B. deineshop@gmail.com
    pass: process.env.MAIL_PASS, // App-Passwort (nicht dein normales PW!)
  },
})
    ...
    <div style="font-family:sans-serif;max-width:560px;margin:auto;color:#333;">
      ...
      <div style="background:#FFF8F2;padding:32px;border:1px solid #f0e0cc;border-top:none;border-radius:0 0 12px 12px;">
        ...
        <p style="margin-top:32px;font-size:13px;color:#aaa;">
          Du hast Fragen? Schreib uns: <a href="mailto:${process.env.MAIL_USER}" style="color:#D4A574;">${process.env.MAIL_USER}</a>
        </p>
      </div>
    </div>
  `
