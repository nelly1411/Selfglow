...
`OPENAI_API_KEY` is used by the keyword RAG chatbot. If it is missing, the
backend still returns product matches with a simple fallback answer, but the
answer will not be generated by the AI model.
...
```json
{
  "message": "Ich habe ölige Haut und suche etwas gegen Unreinheiten."
}
```
