...
// ── Bild komprimieren auf max 800px / 70% Qualität ─────────────────────────
...
export default function SkinAnalysis({ onClose }: SkinAnalysisProps) {
  const [mode,      setMode]      = useState<'choose' | 'camera' | 'preview' | 'loading' | 'result'>('choose')
  ...
  // ── Analyse — komprimiert über Backend ───────────────────────────────────
  ...
  // ── Helpers ───────────────────────────────────────────────────────────────
  const scoreColor = (val: number) => val < 30 ? '#7ab87a' : val < 60 ? '#D4A574' : '#c47a5a'
  const scoreLabel = (val: number) => val < 30 ? 'Niedrig' : val < 60 ? 'Mittel' : 'Hoch'

  const categoryEmoji = (cat: string) => {
    if (cat === 'Serum')               return '💧'
    if (cat === 'Feuchtigkeitspflege') return '🌿'
    if (cat === 'Toner')               return '✨'
    if (cat === 'Sonnenschutz')        return '☀️'
    return '🧴'
  }
      ...
      <div style={{ padding: 24 }}>
        ...
        {mode === 'choose' && (
          <div className="sa-fade-up">
            ...
            {error && (
              <div style={{ background: '#fff0f0', border: '1px solid #fcc', borderRadius: 12, padding: '10px 14px', marginBottom: 16, fontSize: 13, color: '#c47a5a' }}>
                {error}
              </div>
            )}

            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <button onClick={startCamera} className="sa-btn"
                style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 20px', borderRadius: 14, background: '#1c1209', border: 'none', color: '#fff', textAlign: 'left', width: '100%' }}>
          ...
          <div className="sa-fade-up">
            {cameraErr ? (
              <div style={{ textAlign: 'center', padding: '24px 0' }}>
                ...
                <button onClick={reset} className="sa-btn"
                  style={{ padding: '10px 20px', borderRadius: 100, background: '#F5E6D3', border: 'none', color: '#7a5c42', fontSize: 13, fontWeight: 600 }}>
                  Zurück
                </button>
              </div>
            ) : (
              <>
                <div style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', background: '#000', marginBottom: 16 }}>
                  ...
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
                    <div style={{ width: 160, height: 200, borderRadius: '50% 50% 45% 45%', border: '2px solid rgba(212,165,116,0.6)', boxShadow: '0 0 0 1000px rgba(0,0,0,0.3)' }} />
                  </div>
                </div>
                <canvas ref={canvasRef} style={{ display: 'none' }} />
                <p style={{ fontSize: 12, color: '#9a7a5a', textAlign: 'center', margin: '0 0 16px', fontWeight: 300 }}>
                  Positioniere dein Gesicht im Oval
                </p>
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => { stopCamera(); setMode('choose') }} className="sa-btn"
                    style={{ flex: 1, padding: '12px', borderRadius: 100, background: 'transparent', border: '1.5px solid #e0c9a8', color: '#7a5c42', fontSize: 14, fontWeight: 600 }}>
                    Abbrechen
                  </button>
                  <button onClick={takeSelfie} className="sa-btn"
                    style={{ flex: 2, padding: '12px', borderRadius: 100, background: '#D4A574', border: 'none', color: '#fff', fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        ...
        {mode === 'preview' && imageData && (
          <div className="sa-fade-up">
            ...
            <p style={{ fontSize: 13, color: '#9a7a5a', textAlign: 'center', margin: '0 0 16px', fontWeight: 300 }}>
              Sieht gut aus! Bereit für die Analyse?
            </p>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={reset} className="sa-btn"
                style={{ padding: '12px 16px', borderRadius: 100, background: 'transparent', border: '1.5px solid #e0c9a8', color: '#7a5c42', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                <RotateCcw size={14} /> Neu
              </button>
              <button onClick={analyse} className="sa-btn"
                style={{ flex: 1, padding: '12px', borderRadius: 100, background: '#D4A574', border: 'none', color: '#fff', fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          ...
          <div className="sa-fade-up">
            ...
            <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 20 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 22, fontWeight: 800, color: '#1c1209', margin: '0 0 4px', letterSpacing: '-0.02em' }}>{result.skinType}</p>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {result.products.map((p, i) => (
                    <div key={i} className="sa-product-card"
                      style={{ background: '#fff', border: '1px solid #F0DCC8', borderRadius: 14, padding: '12px 16px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={reset} className="sa-btn"
                style={{ padding: '12px 16px', borderRadius: 100, background: 'transparent', border: '1.5px solid #e0c9a8', color: '#7a5c42', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                <RotateCcw size={13} /> Neu
              </button>
              <button
                onClick={() => {
                  // Hauttyp auf Shop-Werte mappen
                  ...
                  // Concerns auf Shop-Werte mappen
                  const concernMap: Record<string, string> = {
                    'redness':   'Rötungen',
                    'blemishes': 'Acne',
                    'dryness':   'Grosse Poren',
                  }
                  const params = new URLSearchParams()
                  params.set('skinType', mappedSkinType)

                  // Alle Concerns mit Wert >= 50 hinzufügen
                  ...
                  activeConcerns.forEach(c => {
                    params.append('concern', concernMap[c.key])
                  })
